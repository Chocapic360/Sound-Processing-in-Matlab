% Modified script of part E function that takes the m4a file in the
% directory and runs the frequency estimation function on it
clc, clear
...
toneDuration = 0.1;  % Duration of padding in seconds
[audio_data, fs] = audioread('5_key_from_phone.m4a');  % fs is sampling rate
timearray = 0:1/fs:toneDuration;

% Convert audio_data to row vector
waveform = audio_data(:)';

% Calculate number of padding samples
paddingSamples = round(fs * toneDuration / 2);

% Create padding
padded_waveform = [zeros(1, paddingSamples), waveform, zeros(1, paddingSamples)];
...
if(minDistancef1 < tolerance && minDistancef2 < tolerance) % Check if the closest values are within the tolerance
    ...
    fprintf('Detected frequencies: %d Hz and %d Hz\n',f1_est,f2_est);
    fprintf('Estimated DTMF frequencies: %d Hz and %d Hz\n',f1_round,f2_round);
    subplot(2,2,4);
    hold on;
